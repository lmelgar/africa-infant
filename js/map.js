...
var infantmort = "total"; // illness to map / plot
...
var linechart = d3.select("#graph2")
			.append("svg")
			.attr("width", width)
			.attr("height", height);
...
queue()
  ...
  .defer(d3.csv, "data/my2015.csv", typeAndSet)
  .defer(d3.csv, "data/infantmort.csv", typeAndSet) // process
...
function update_lines(country, data) {
  ...
  // transition.  there is no need for enter and exit in this case. we entered when we drew the first one in draw_lines.
	...
	if (data.length && data[0].total > 0) {
		d3.select(".subhead").html("Historical Rate for " + countryById.get(country).country);
	} else if (!data.length || data[0]total == 0) {
...
function getColor(d) {
    ...
    if (dataRow) {
        return mapColorScale(+dataRow.total);
    } else {
